...
def stall_car(stall, car):
    ...
    for a in stall:
        ...
        print("右下角坐标：", a[3])
        # 排 model
        if car.__len__() > count and site == 'leftUp':
            las = []
            start_x0 = a[1][0]
            start_y0 = a[1][1]
            for model_num in range(model_nums.__len__()):
                model_name = model_nums[model_num]
                print(model_name)
                # model1
                if car[count].get(model_name).__len__() != 0:
                    la = {"modelKey": model_name}
                    cood = []
                    if cood.__len__() != 0:
                        start_x0 = cood[cood.__len__()][2][0]
                    nu = 0
                    for model1_num_y in range(car[count].get(model_name)[2][0]):
                        start_x = start_x0
                        start_y = start_y0 - nu * M[model_name][0]
                        nu += 1
                        # x方向model1排布车位
                        for model1_num_x in range(car[count].get(model_name)[1][0]):
                            # 坐标数组
                            coord = []
                            # 左下角坐标
                            leftDown = [start_x, start_y - M[model_name][0]]
                            # 左上角坐标
                            leftUp = [start_x, start_y]
                            # 右上角
                            rightUp = [start_x + M[model_name][1], start_y]
                            # 右下角
                            rightDown = [start_x + M[model_name][1], a[1][1] - M[model_name][0]]
                            coord.append(leftDown)
                            coord.append(leftUp)
                            coord.append(rightUp)
                            coord.append(rightDown)
                            cood.append(coord)
                            start_x = start_x + M[model_name][1]
                        start_x0 = start_x
                    la["cood"] = cood
                    las.append(la)
            last[str(count)] = las
